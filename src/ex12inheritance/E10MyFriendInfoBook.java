...
class HighFriend extends Friend{
	...
	/*
	(오버라이딩 1)
	고딩친구의 전체정보를 출력하기 위해 부모에서 정의한 메서드를 super를 통해
	...
	*/
	...
	/*
	(오버라이딩 2)
	자식 클래스에서 오버라이딩하여 재정의한 메서드로, 고딩친구의 간략한 정보를 출력한다.
	부모 클래스에서는 실행부가 없는 상태로 정의되었다.
	*/
	@Override
	public void showBasicInfo() {
...
public class E10MyFriendInfoBook {
	...
	*/
	...
	자주 사용한다.
	main메서드에서는 특별한 기능은 구현하지 않고 프로그램 전반의 흐름에
	대해서만 담당한다. 
	*/
...
/*
해당 프로그램에서 기능을 담당하는 클래스로 핸들러 혹은 매니저클래스 라고 부른다.
*/
class FriendInfoHandler{
	...
	*/
	...
	public void addFriend(int choice) {
		//System.out.println("addrFriend 호출됨");
		...
		if(choice==1) {
			...
			/*
			참조변수를 인스턴스 배열에 추가한다.
			인덱스로 사용된 변수의 초기값이 0이므로 0번 인덱스에 추가한뒤
			1증가시킨다.
			이를 위해 '후위증가' 하고있다.
			*/
		...
		System.out.println("친구정보 입력이 완룓되었습니다.");
		/*
		더이상 실행할 문장이 없으므로 해당 메서드는 메모리에서 소멸되고 호출한
		지점인 main 메서드로 돌아간다.
		*/
	}//end of addFriend
	...
	*/
	...
	public void showSimpleData() {
		...
		/*
		만약 부모클래스인 Friend에 showBasicInfo()메서드를 정의하지
		않으면 자식클래스에서는 오버라이딩을 할 수 없으므로 개별적인
		멤버메서드를 정의해야한다.
	...
	public void deleteInfo() {
		...
		*/
		...
		if(deleteIndex==-1) {
			...
			/*
			인스턴스 배열에서 원소를 삭제한 후 바로 뒤에 있는 원소를 앞으로
			하나씩 복사한다.
			만약 이 부분이 처리되지 않는다면 차후 검색이나 삭제시 
