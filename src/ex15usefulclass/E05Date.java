...
public class E05Date {
	...
	*/
	public static String myDateFormat(Calendar c, String formatStr) {
		/*
		getTime() : 서버의 표준시인 1970-01-01 부터 현재까지의 시간을 초 단위로
		반환해준다.
		이를 타임스템프라고 한다.
		*/
		Date date = c.getTime();
	...
	public static void main(String[] args) {
		...
		/*
		Calendar 클래스는 태표적인 싱글톤 객체이므로 new로 인스턴스를 생성할 수 없고
		유틸리티 메서드를 통해 참조값을 얻어온다.
		*/
		Calendar calendar = Calendar.getInstance();
		/*
		월을 표현하는 Month, 분을 표현하는 Minute의 이니셜이 동일하기 떄문에
		구분을위해 월은 대문자, 분은 소문자를 사용한다.
		*/
		String nowStr1 = myDateFormat(calendar, "yyyy-MM-dd HH:mm:ss");
		System.out.println("현재1:"+ nowStr1);
		//그 외 여러가지 서식문자가 존재한다.
		String nowStr2 = myDateFormat(calendar, "오늘은 M월의 W번째주, d번째날, F번쨰 E요일");
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			*/
			...
			순수한 초(Sec)이고, 11~13자리는 멀티세컨즈를 의미한다.
			즉, 100으로 나눠서 위에 3자리를 제거하면 순수한 초가 반환된다.
