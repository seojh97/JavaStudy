...
/*
쓰레드로 구현하지 않은 일반적인 클래스 : 하나의 클래스가 작업을 완료해야 다음 클래스의
작업을 진행할 수 있다.
즉, 동시에 작업을 진행할 수 없다.
*/
class NormalClass{
...
/*
독립쓰레드(Non Daemon Thread) : 메인쓰레드와 
Working쓰레드(개발자가 정의한 쓰레드)를 일컫는 것으로 메인쓰레드가 종료 되어도 
계속 실행되며, Dead상태가 되어야만 실행이 종료되는 쓰레드를 말한다.
...
class NormalThread extends Thread{
	...
	*/
	...
	지정할 수 있다.
	이때 지정된 이름은 getName()을 통해 반환 받을 수 있다.
	*/
	public NormalThread(String title) {
		super(title);
	}
	/*
	쓰레드 동작 중 비정상적으로 종료될때 InterruptedException이
	발생할 수 있으므로, 예외처리를 해야한다.
	여기서는 즉시 처리 하지않고 throws해서 호출한 위치로 예외객체를 던지기한다. 
	*/
...
/*
종속쓰레드(Daemon Thread) : 모든 독립쓰레드가 종료되면 자동으로 종료되는
쓰레드를 말한다.
메인메서드도 하나의 독립쓰레드 이므로 다른 독립쓰레드가 남아있다면 종료 될때까지는
포로그램이 종료된 것이 아니다.
주로 프로그램의 보조역할로 배경음악재생, 로그자동저장 등의 업무를 처리한다.
프로그램이 종료 될때 함께 종료 되므로 무한루프로 구현한다. 
*/
