...
/*
쓰레드 클래스를 생성하는 두번째 방법 : Java는 다중상속을 지원하지 않으므로
기능을 위해 다른 클래스를 상속 해야한다면 Runnable 인터페이스를 구현해서 
쓰레드 클래스를 정의 해야한다.
*/
class AdderThread extends Sum implements Runnable{
	...
	누적합을 구한다.
	start가 1이고 end가 10이라면 1~10까지 합을 쓰레드를 통해 계산 하면된다.
	*/
	@Override 
	public void run() {
...
public class Ex04Runnable {
	...
	public static void main(String[] args) {
		...
		쓰레드를 생성한다.
		해당 방법도 결국 Thread클래스를 통해서 쓰레드를 생성 하게된다.
		*/
		...
		try {
			...
			즉, 해당 쓰레드가 하나의 작업이 종료 되기를 기다렸다가 다음 쓰레드를 실행한다.
			쓰레드의 동시접근을 제한하는 기능을 가지고있다.
			*/
			thread1.join();
			thread2.join();
		}
